...
interface AdultHomeNewsConfigProps {
    initialSettings: Record<string, any>;
    availableCourses: Course[];
}

export default function AdultHomeNewsConfig({ initialSettings, availableCourses }: AdultHomeNewsConfigProps) {
    const [loading, setLoading] = useState(false);

    // Attempt to parse existing configuration or default to empty array
    let parsedInitial = [];
    try {
        if (initialSettings.adult_home_news_courses) {
            parsedInitial = JSON.parse(initialSettings.adult_home_news_courses);
    ...
    const handleSave = async () => {
        ...
        try {
            await saveAdultHomeSettings({
                news_courses: selectedIds // Will be saved as 'adult_home_news_courses'
            });
            alert("Nouveautés enregistrées avec succès !");
        } catch (error) {
            console.error(error);
    ...
    const toggleSelection = (courseId: string) => {
        if (selectedIds.includes(courseId)) {
            ...
            if (selectedIds.length >= 3) {
                alert("Vous ne pouvez sélectionner que 3 nouveautés maximum.");
                return;
            }
        ...
        <Card className="bg-black border-white/5 shadow-2xl overflow-hidden mt-8">
            ...
            <CardContent className="p-8">
                ...
                <p className="text-sm text-slate-400 mb-8 font-light max-w-3xl">
                    Sélectionnez ici <strong>exactement 3 vidéos</strong> qui doivent apparaître dans la section "Nouveautés de la Semaine" sur la page d'accueil de L'Atelier. Idéal pour remonter une ancienne Masterclass au sommet de l'actualité ! Si vous n'en sélectionnez aucune, les 3 dernières vidéos publiées s'afficheront par défaut.
                </p>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                    ...
                    <div className="space-y-4">
                        <h3 className="text-sm font-bold uppercase tracking-widest text-magic-gold flex items-center gap-2">
                            Mises en avant ({selectedCourses.length}/3)
                        </h3>

...
export default function LivePage() {
    ...
    return (
        <div className="min-h-screen bg-[#050507] text-white p-4 md:p-8 relative font-sans selection:bg-magic-cyan/30">
            {/* Ambient Background Lights (Adult Theme) */}
            <div className="absolute top-[-20%] left-[-10%] w-[60%] h-[60%] bg-magic-cyan/5 blur-[150px] rounded-full pointer-events-none mix-blend-screen z-0"></div>
            <div className="absolute top-[30%] right-[-10%] w-[40%] h-[40%] bg-blue-700/5 blur-[120px] rounded-full pointer-events-none mix-blend-screen z-0"></div>

            <div className="max-w-6xl mx-auto space-y-12 relative z-10">

                {/* ACTIVE / NEXT LIVE SECTION */}
                <section className="text-center space-y-8 mt-8">
                    <div className="mx-auto w-24 h-24 bg-magic-cyan/10 rounded-full flex items-center justify-center border border-magic-cyan/30 shadow-[0_0_50px_rgba(238,195,67,0.2)] animate-pulse">
                        <Video className="w-12 h-12 text-magic-cyan" />
                    </div>

                    <h1 className="text-4xl font-serif font-bold bg-clip-text text-transparent bg-gradient-to-r from-magic-cyan to-blue-500">
                        {live?.status === 'en_cours' ? '🔴 LIVE EN COURS !' : 'Salle de Visio Magique'}
                    </h1>

                    {live ? (
                        <div className="max-w-xl mx-auto bg-magic-card border border-magic-cyan/30 p-8 rounded-3xl shadow-2xl relative overflow-hidden">
                ...
                {replays.length > 0 && (
                    <section>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {replays.map((replay) => (
                                <div key={replay.id} className="bg-magic-card border border-white/10 rounded-xl overflow-hidden group hover:border-magic-cyan/50 transition-all">
                                    ...
                                    <div className="p-4">
                                        <h3 className="font-bold text-lg mb-1 group-hover:text-magic-cyan transition-colors">{replay.title}</h3>

...
async function checkIphone() {
    ...
    try {
        ...
        if (iframeRes.status !== 200) {
            console.log("FAILED: Iframe is blocked. This means the 'Embed view token authentication' is rejecting the key we generate! The Token Key in Vercel MUST perfectly match the one in Bunny.");
            return;
        }

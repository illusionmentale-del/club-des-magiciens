...
export default function AdultBoutiqueCard({ product, isPurchased }: AdultBoutiqueCardProps) {
    ...
    return (
        <div className="relative group h-full flex flex-col hover:-translate-y-2 transition-transform duration-500">
            ...
            <div className="absolute -inset-1 bg-gradient-to-r from-magic-cyan/80 to-amber-600/50 rounded-3xl opacity-0 blur-lg group-hover:opacity-40 transition duration-1000 pointer-events-none"></div>
            ...
            <div className="relative bg-[#0a0a0f] border border-white/5 rounded-3xl overflow-hidden shadow-[0_10px_40px_rgba(0,0,0,0.5)] flex flex-col h-full">
                ...
                <div className="relative aspect-video bg-[#111] overflow-hidden">
                    ...
                    <div className="absolute top-4 right-4 z-10">
                        {isPurchased ? (
                            ...
                            <div className="bg-black/60 backdrop-blur-md text-white px-3 py-1.5 rounded-full text-xs font-bold flex items-center gap-1.5 shadow-lg border border-white/10">
                                <Lock className="w-3.5 h-3.5 text-magic-cyan" />
                                Premium
                            </div>
                        )}
                    </div>
                    ...
                    {/* Price Tag (if locked) */}
                    {!isPurchased && (
                        <div className="absolute bottom-4 left-4 bg-magic-cyan text-black font-black px-4 py-1.5 rounded-xl shadow-xl transform -rotate-2 z-10">
                            {priceString}
                        </div>
                    )}
                </div>
                ...
                <div className="p-6 flex flex-col flex-1 relative z-20 bg-[#0a0a0f]">
                    <h3 className="text-xl font-bold text-white mb-2 leading-tight group-hover:text-magic-cyan transition-colors">
                        {product.title}
                    </h3>
                    ...
                    {/* Action Button */}
                    {isPurchased ? (
                        <Link href={`/dashboard/program`} className="w-full bg-white/5 hover:bg-white/10 text-white font-bold py-3 px-4 rounded-xl flex items-center justify-center gap-2 transition-colors border border-white/10 hover:border-magic-cyan/50">
                            <Play className="w-5 h-5 text-magic-cyan" />
                            Accéder au contenu
                        </Link>
                    ) : (
                        <button
                            ...
                            className="w-full bg-gradient-to-r from-magic-cyan to-yellow-500 text-black font-black py-3 px-4 rounded-xl flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform shadow-[0_10px_30px_rgba(238,195,67,0.2)] disabled:opacity-50 disabled:hover:scale-100"
                        >
                            {loading ? (

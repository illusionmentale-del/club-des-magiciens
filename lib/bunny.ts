...
export async function getSecureBunnyIframeUrl(libraryId: string | number, videoId: string, isKid: boolean = false): Promise<string> {
    ...
    if (!tokenKey) {
        ...
        return `https://player.mediadelivery.net/embed/${libraryId}/${videoId}?autoplay=true&loop=false&muted=false&preload=true&responsive=true`;
    }
    ...
    return `https://player.mediadelivery.net/embed/${libraryId}/${videoId}?token=${hash}&expires=${expires}&autoplay=true&loop=false&muted=false&preload=true&responsive=true`;
}

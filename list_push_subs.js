...
async function check() {
  ...
  const { data: profiles } = await supabase.from('profiles').select('id, full_name, email').in('id', uniqueUserIds);
  
  console.log(`Total unique users with push subscriptions: ${uniqueUserIds.length}`);

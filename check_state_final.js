...
async function checkFinalState() {
    ...
    try {
        ...
        if (iframeRes.status !== 200) {
            console.log("FAILED: Iframe is blocked. Token Auth configuration is incorrect on Bunny Dashboard.");
            return;
        }
        ...
        if (!m3u8Match) {
            console.log("FAILED: Cannot find m3u8 playlist in the iframe HTML. The player might be returning an error page.");
        ...
        if (assetNoRef.status === 403 && assetWithRef.status === 200) {
             console.log("\nCONCLUSION: 'CDN Token Authentication' or 'Block Direct Access' is STILL ENABLED on Bunny.net! It blocks requests lacking a Referer (Safari).");
        } else if (assetNoRef.status === 200 && assetWithRef.status === 200) {
             console.log("\nCONCLUSION: The CDN security is correctly configured. Assets are accessible. If it still fails, it's a front-end UI issue.");
        } else if (assetNoRef.status === 403 && assetWithRef.status === 403) {
             console.log("\nCONCLUSION: Assets are completely blocked. The internal CDN tokens generated by the iframe might be rejecting the request.");
        }
